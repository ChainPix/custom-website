...
test("computeNextRuns returns deterministic next runs in UTC", async () => {
  ...
  await expect(iso).toMatchFileSnapshot("./__snapshots__/cron-tester/cron-next-runs.txt");
});
...
test("snapshot: known cron examples", async () => {
  const examples = [
    { expr: "0 * * * *", label: "hourly" },
  ...
  await expect(payload).toMatchFileSnapshot("./__snapshots__/cron-tester/cron-examples.json");
});
