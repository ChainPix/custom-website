...
function loadParser() {
  ...
  return moduleShim.exports as any;
}
...
test("curl-to-fetch snapshots stay stable", () => {
  ...
  for (let i = 0; i < snapshots.length; i += 1) {
    ...
    // Fixtures go through JSON.stringify, which drops undefined-valued keys;
    // normalize the runtime object the same way before comparing.
    const normalizedParse = JSON.parse(JSON.stringify(parsed));
    assert.deepEqual(normalizedParse, snapshot.parse, `parse mismatch for ${snapshot.name}`);
