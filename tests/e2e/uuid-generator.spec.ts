...
test("uuid generator flags invalid count", async ({ page }) => {
  ...
  // Filter out Next.js's empty route announcer, which also has role="alert".
  await expect(page.getByRole("alert").filter({ hasText: /count/i })).toContainText(
    "Enter a count between 1 and 50."
  );
});
