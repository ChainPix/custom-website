 ...
 * - app/sitemap.ts (sitemap priorities)
 * - tests/unit/tools-registry.spec.ts (registry ↔ filesystem consistency)
 * - tests/e2e/all-tools.smoke.spec.ts (smoke test over every tool page)
 *
 * `slug` includes the leading slash and MUST match the directory name under
 * app/(tools)/ (it is also persisted in localStorage by the homepage
 * "recently used" feature, so do not change existing slugs).
 * `featured` tools get priority 0.9/weekly in the sitemap; others 0.8/monthly.
 */
export type Tool = {
  slug: string;
  title: string;
...
/** Path of a tool page, e.g. "/json-formatter". */
export const toolPath = (tool: Tool | string): string =>
  typeof tool === "string" ? tool : tool.slug;

/** Absolute URL of a tool page on the production site. */
export const toolUrl = (tool: Tool | string): string =>
  `${siteUrl.replace(/\/$/, "")}${toolPath(tool)}`;
