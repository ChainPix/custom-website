/**
 * Pure text-case conversion logic shared by the text-case worker and covered
 * by tests/unit/text-case.spec.ts. Locale-aware, acronym-preserving, with
 * smart number splitting and punctuation retention modes.
 */
...
export const tokenize = (text: string, options: ConverterOptions): Token[] => {
  const tokens: Token[] = [];
  let current = "";
...
export const isAcronymToken = (value: string, locale: string) => {
  const letters = getLetters(value);
  if (letters.length < 2) return false;
  return letters === letters.toLocaleUpperCase(locale);
};

export const capitalizeWord = (value: string, locale: string) => {
  let result = "";
  let upperNext = true;
...
export const toStudly = (value: string, locale: string) => {
  let result = "";
  let upperNext = true;
  for (const char of value) {
    if (isLetter(char)) {
...
export const convertText = (text: string, caseType: CaseType, options: ConverterOptions) => {
  if (!text) return "";
  if (caseType === "upper") {
...
export const convertTextWithLineMode = (text: string, caseType: CaseType, options: ConverterOptions) => {
  if (!options.perLine) {
