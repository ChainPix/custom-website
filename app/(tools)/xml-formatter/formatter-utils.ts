...
export const extractErrorLocation = (message: string): XmlParseLocation | null => {
  const patterns = [
...
export const formatXml = (payload: FormatPayload) => {
  ...
  const finalOutput =
    xmlDeclaration && !output.startsWith("<?xml")
      ? output
        ...
        : xmlDeclaration
      : output;
  return { output: finalOutput, summary };
};
