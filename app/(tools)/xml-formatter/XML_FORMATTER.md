...
## Current state
- Features: Paste XML, validate via DOMParser, and beautify via DOM tree traversal (preserves mixed content, CDATA, comments, processing instructions, DOCTYPE) with 2/4 space indent; copy/download output; reset to sample; guards for empty/very large input (~200KB); aria-live status and labeled regions; metadata + FAQPage JSON-LD; on-page How-to/privacy note.
