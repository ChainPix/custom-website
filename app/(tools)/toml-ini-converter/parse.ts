/**
 * Pure TOML/INI/JSON conversion logic shared by the toml-ini worker and
 * covered by tests/unit/toml-ini-converter.spec.ts. Kept free of worker
 * globals so it can be imported and unit-tested directly.
 */
...
export type Mode = "toml" | "ini" | "json";
...
export const findIniLineError = (raw: string) => {
...
export const escapeIniSections = (raw: string) =>
...
export const getLineColumn = (text: string, offset: number): ErrorLocation => {
...
export const extractJsonErrorLocation = (message: string, raw: string): ErrorLocation | null => {
...
export const validateSchema = (schemaInput: string, value: unknown): SchemaValidation => {
...
export const coerceIniPrimitive = (value: string) => {
...
export const transformIniValue = (
  value: unknown,
...
export const parseInput = (message: ParseRequest): ParseResponse => {
  const {
    input,
    mode,
    outputFormat,
